  ...
  return {
    compile = function(args)
      ...
      vim.schedule(function()
        client:request("java/buildWorkspace", { forceRebuild = args.compile_mode == "full" }, function(err, result)
          logger.warn(("[timing] java/buildWorkspace callback after %.1f ms"):format((vim.uv.hrtime() - started) / 1e6))
          if err then
