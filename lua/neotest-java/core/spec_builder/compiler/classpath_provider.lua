    ...
    get_classpath = function(base_dir, additional_classpath_entries)
      ...
      client:request("workspace/executeCommand", {
        ...
        if err then
          ...
          logger.warn(("[timing] java.project.getClasspaths(runtime) finished in %.1f ms"):format((vim.uv.hrtime() - started) / 1e6))
      ...
      client:request("workspace/executeCommand", {
        ...
        if err then
          ...
          logger.warn(("[timing] java.project.getClasspaths(test) finished in %.1f ms"):format((vim.uv.hrtime() - started) / 1e6))
          test.set(result.classpaths)
        end
      end, bufnr)

      local additional = vim
